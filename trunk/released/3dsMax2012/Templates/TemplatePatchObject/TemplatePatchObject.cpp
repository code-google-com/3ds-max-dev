...
int TemplatePatchObjectClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplatePatchObjectClassDesc::Create(BOOL loading) 
{ 
    return new TemplatePatchObject(); 
} 
const MCHAR* TemplatePatchObjectClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Patch Object"); 
} 
SClass_ID TemplatePatchObjectClassDesc::SuperClassID() 
{ 
    return GEOMOBJECT_CLASS_ID;
} 
Class_ID TemplatePatchObjectClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x66F5A74C, 0x7E88F872);
    return cid; 
} 
const MCHAR* TemplatePatchObjectClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplatePatchObjectClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplatePatchObject"); 
} 
HINSTANCE TemplatePatchObjectClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplatePatchObjectClassDesc::GetClassDescInstance() 
{
    static TemplatePatchObjectClassDesc desc;
    return &desc;
}
...
class TemplatePatchObjectParams
    : public ParamBlockDescUtil
{
public:        
    TemplatePatchObjectParams()
        : ParamBlockDescUtil(TemplatePatchObjectClassDesc::GetClassDescInstance(), _T("TemplatePatchObjectParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplatePatchObject::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplatePatchObject::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplatePatchObjectParams pblockdesc;
...
{ 
	return "Patch Object template plug-in project."; 
} 
...
{ 
    ...
    static TemplatePatchObjectClassDesc classdesc; 
    return &classdesc; 
} 
