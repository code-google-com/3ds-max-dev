...
int TemplateControllerClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateControllerClassDesc::Create(BOOL loading) 
{ 
    return new TemplateController(); 
} 
const MCHAR* TemplateControllerClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Controller"); 
} 
SClass_ID TemplateControllerClassDesc::SuperClassID() 
{ 
    return CTRL_POSITION_CLASS_ID;
} 
Class_ID TemplateControllerClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x153B2A18, 0x0A668F09);
    return cid; 
} 
const MCHAR* TemplateControllerClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateControllerClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateController"); 
} 
HINSTANCE TemplateControllerClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateControllerClassDesc::GetClassDescInstance() 
{
    static TemplateControllerClassDesc desc;
    return &desc;
}
...
class TemplateControllerParams
    : public ParamBlockDescUtil
{
public:        
    TemplateControllerParams()
        : ParamBlockDescUtil(TemplateControllerClassDesc::GetClassDescInstance(), _T("TemplateControllerParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateController::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateController::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateControllerParams pblockdesc;
...
{ 
	return "Controller template plug-in project."; 
} 
...
{ 
    ...
    static TemplateControllerClassDesc classdesc; 
    return &classdesc; 
} 
