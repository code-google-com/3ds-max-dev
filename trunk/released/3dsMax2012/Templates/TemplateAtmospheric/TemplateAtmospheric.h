...
class TemplateAtmosphericClassDesc
    : public ClassDesc2 
{
public: 
    //---------------------------------------
...
class TemplateAtmospheric 
    : public FXPlugin<Atmospheric>
{
    //============================================================
    // Fields
    ...
    static const int TemplateAtmosphericHeaderChunkID = 1;
...
public:
    ...
    TemplateAtmospheric()
		: FXPlugin<Atmospheric>(TemplateAtmosphericClassDesc::GetClassDescInstance())
    {         
    ...
    { 
	    IOResult res;
	    isave->BeginChunk(TemplateAtmosphericHeaderChunkID);
        
        TODO("Save any member fields");

        res = FXPlugin<Atmospheric>::Save(isave);
    ...
    { 
        ...
        while (iload->OpenChunk() == IO_OK) 
        {
		    ...
		    switch(id)  
            {
			    case TemplateAtmosphericHeaderChunkID:
        
                    TODO("Load any member fields");

                    res = FXPlugin<Atmospheric>::Load(iload);
				    break;
			}
		    iload->CloseChunk();
    ...
    {
        ReferenceTarget* r = new TemplateAtmospheric();
    ...
    virtual void BaseClone(ReferenceTarget* from, ReferenceTarget* to, RemapDir& remap)  
    {
        ...
        TemplateAtmospheric* source = dynamic_cast<TemplateAtmospheric*>(from);
        TemplateAtmospheric* target = dynamic_cast<TemplateAtmospheric*>(to);
        ...
        FXPlugin<Atmospheric>::BaseClone(from, to, remap);
    }

    //=========================================================================
    // FXPlugin<Atmospheric> overrides 

    virtual void Shade(ShadeContext& sc, const Point3& p0, const Point3& p1, Color& color, Color& trans, BOOL isBG = FALSE)
    {
        TODO("Apply atmospheric effect to shade context");
    }
    ...
    {
        return TemplateAtmosphericClassDesc::GetClassDescInstance();
    }
