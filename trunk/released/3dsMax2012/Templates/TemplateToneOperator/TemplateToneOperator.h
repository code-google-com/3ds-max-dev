...
class TemplateToneOperatorClassDesc
    : public ClassDesc2 
{
public: 
    //---------------------------------------
...
class TemplateToneOperator 
    : public FXPlugin<ToneOperator>
{
    //============================================================
    // Fields
    ...
    static const int TemplateToneOperatorHeaderChunkID = 1;
...
public:
    ...
    TemplateToneOperator()
		: FXPlugin<ToneOperator>(TemplateToneOperatorClassDesc::GetClassDescInstance())
    {         
    ...
    { 
	    IOResult res;
	    isave->BeginChunk(TemplateToneOperatorHeaderChunkID);
        
        TODO("Save any member fields");

        res = FXPlugin<ToneOperator>::Save(isave);
    ...
    { 
        ...
        while (iload->OpenChunk() == IO_OK) 
        {
		    ...
		    switch(id)  
            {
			    case TemplateToneOperatorHeaderChunkID:
        
                    TODO("Load any member fields");

                    res = FXPlugin<ToneOperator>::Load(iload);
				    break;
			}
		    iload->CloseChunk();
    ...
    {
        ReferenceTarget* r = new TemplateToneOperator();
    ...
    virtual void BaseClone(ReferenceTarget* from, ReferenceTarget* to, RemapDir& remap)  
    {
        ...
        TemplateToneOperator* source = dynamic_cast<TemplateToneOperator*>(from);
        TemplateToneOperator* target = dynamic_cast<TemplateToneOperator*>(to);
        ...
        FXPlugin<ToneOperator>::BaseClone(from, to, remap);
    }

    //=========================================================================
    // FXPlugin<ToneOperator> overrides 

    virtual void ScaledToRGB (float energy[3])
    {
        TODO("Map a scaled energy value to monochrome.");
    }
    ...
    {
        TODO("Return the physical value scaled to 1.");
        return 1.0f;
    }
    virtual void SetPhysicalUnit (float value, TimeValue t)
    {
        TODO("Set the phsyical value that is scaled to 1.");
    }
    virtual void ScalePhysical (float energy[3]) const
    {
        TODO("Scale physical values so they can be used in the renderer.");
    }
    ...
    {
        return TemplateToneOperatorClassDesc::GetClassDescInstance();
    }
