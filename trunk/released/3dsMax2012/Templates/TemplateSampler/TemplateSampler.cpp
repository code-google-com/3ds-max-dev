...
int TemplateSamplerClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateSamplerClassDesc::Create(BOOL loading) 
{ 
    return new TemplateSampler(); 
} 
const MCHAR* TemplateSamplerClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Sampler"); 
} 
SClass_ID TemplateSamplerClassDesc::SuperClassID() 
{ 
    return SAMPLER_CLASS_ID;
} 
Class_ID TemplateSamplerClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x133A0C69, 0x3DBC1672);
    return cid; 
} 
const MCHAR* TemplateSamplerClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateSamplerClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateSampler"); 
} 
HINSTANCE TemplateSamplerClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateSamplerClassDesc::GetClassDescInstance() 
{
    static TemplateSamplerClassDesc desc;
    return &desc;
}
...
class TemplateSamplerParams
    : public ParamBlockDescUtil
{
public:        
    TemplateSamplerParams()
        : ParamBlockDescUtil(TemplateSamplerClassDesc::GetClassDescInstance(), _T("TemplateSamplerParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateSampler::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateSampler::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateSamplerParams pblockdesc;
...
{ 
	return "Sampler template plug-in project."; 
} 
...
{ 
    ...
    static TemplateSamplerClassDesc classdesc; 
    return &classdesc; 
} 
