...
int TemplateSoundObjectClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateSoundObjectClassDesc::Create(BOOL loading) 
{ 
    return new TemplateSoundObject(); 
} 
const MCHAR* TemplateSoundObjectClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Sound Object"); 
} 
SClass_ID TemplateSoundObjectClassDesc::SuperClassID() 
{ 
    return SOUNDOBJ_CLASS_ID;
} 
Class_ID TemplateSoundObjectClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x6C859D36, 0x6CB54C3E);
    return cid; 
} 
const MCHAR* TemplateSoundObjectClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateSoundObjectClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateSoundObject"); 
} 
HINSTANCE TemplateSoundObjectClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateSoundObjectClassDesc::GetClassDescInstance() 
{
    static TemplateSoundObjectClassDesc desc;
    return &desc;
}
...
class TemplateSoundObjectParams
    : public ParamBlockDescUtil
{
public:        
    TemplateSoundObjectParams()
        : ParamBlockDescUtil(TemplateSoundObjectClassDesc::GetClassDescInstance(), _T("TemplateSoundObjectParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateSoundObject::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateSoundObject::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateSoundObjectParams pblockdesc;
...
{ 
	return "Sound Object template plug-in project."; 
} 
...
{ 
    ...
    static TemplateSoundObjectClassDesc classdesc; 
    return &classdesc; 
} 
