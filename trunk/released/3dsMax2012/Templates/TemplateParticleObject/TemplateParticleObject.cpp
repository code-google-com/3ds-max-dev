...
#include "TemplateParticleObject.h"
...
int TemplateParticleObjectClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateParticleObjectClassDesc::Create(BOOL loading) 
{ 
    return new TemplateParticleObject(); 
} 
const MCHAR* TemplateParticleObjectClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Particle Object"); 
} 
SClass_ID TemplateParticleObjectClassDesc::SuperClassID() 
{ 
    return GEOMOBJECT_CLASS_ID;
} 
Class_ID TemplateParticleObjectClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x42328B82, 0x03D5A0D1);
    return cid; 
} 
const MCHAR* TemplateParticleObjectClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateParticleObjectClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateParticleObject"); 
} 
HINSTANCE TemplateParticleObjectClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateParticleObjectClassDesc::GetClassDescInstance() 
{
    static TemplateParticleObjectClassDesc desc;
    return &desc;
}
...
class TemplateParticleObjectParams
    : public ParamBlockDescUtil
{
public:        
    TemplateParticleObjectParams()
        : ParamBlockDescUtil(TemplateParticleObjectClassDesc::GetClassDescInstance(), _T("TemplateParticleObjectParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateParticleObject::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateParticleObject::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateParticleObjectParams pblockdesc;
...
{ 
	return "Particle Object template plug-in project."; 
} 
...
{ 
    ...
    static TemplateParticleObjectClassDesc classdesc; 
    return &classdesc; 
} 
