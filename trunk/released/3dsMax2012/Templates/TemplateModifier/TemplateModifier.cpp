...
int TemplateModifierClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateModifierClassDesc::Create(BOOL loading) 
{ 
    return new TemplateModifier(); 
} 
const MCHAR* TemplateModifierClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Modifier"); 
} 
SClass_ID TemplateModifierClassDesc::SuperClassID() 
{ 
    return OSM_CLASS_ID;
} 
Class_ID TemplateModifierClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x7F2F1658, 0x0C89B294);
    return cid; 
} 
const MCHAR* TemplateModifierClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateModifierClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateModifier"); 
} 
HINSTANCE TemplateModifierClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateModifierClassDesc::GetClassDescInstance() 
{
    static TemplateModifierClassDesc desc;
    return &desc;
}
...
class TemplateModifierParams
    : public ParamBlockDescUtil
{
public:        
    TemplateModifierParams()
        : ParamBlockDescUtil(TemplateModifierClassDesc::GetClassDescInstance(), _T("TemplateModifierParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateModifier::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateModifier::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateModifierParams pblockdesc;
...
{ 
	return "Modifier template plug-in project."; 
} 
...
{ 
    ...
    static TemplateModifierClassDesc classdesc; 
    return &classdesc; 
} 
