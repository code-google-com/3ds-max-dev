...
int TemplateSplineShapeClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateSplineShapeClassDesc::Create(BOOL loading) 
{ 
    return new TemplateSplineShape(); 
} 
const MCHAR* TemplateSplineShapeClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Spline Shape"); 
} 
SClass_ID TemplateSplineShapeClassDesc::SuperClassID() 
{ 
    return SHAPE_CLASS_ID;
} 
Class_ID TemplateSplineShapeClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x70D16907, 0x75EC0922);
    return cid; 
} 
const MCHAR* TemplateSplineShapeClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateSplineShapeClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateSplineShape"); 
} 
HINSTANCE TemplateSplineShapeClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateSplineShapeClassDesc::GetClassDescInstance() 
{
    static TemplateSplineShapeClassDesc desc;
    return &desc;
}
...
class TemplateSplineShapeParams
    : public ParamBlockDescUtil
{
public:        
    TemplateSplineShapeParams()
        : ParamBlockDescUtil(TemplateSplineShapeClassDesc::GetClassDescInstance(), _T("TemplateSplineShapeParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateSplineShape::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateSplineShape::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateSplineShapeParams pblockdesc;
...
{ 
	return "Spline Shape template plug-in project."; 
} 
...
{ 
    ...
    static TemplateSplineShapeClassDesc classdesc; 
    return &classdesc; 
} 
