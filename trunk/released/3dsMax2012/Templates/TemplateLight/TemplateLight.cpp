...
int TemplateLightClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateLightClassDesc::Create(BOOL loading) 
{ 
    return new TemplateLight(); 
} 
const MCHAR* TemplateLightClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Light"); 
} 
SClass_ID TemplateLightClassDesc::SuperClassID() 
{ 
    return LIGHT_CLASS_ID;
} 
Class_ID TemplateLightClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x16E419D6, 0x19BE1FCB);
    return cid; 
} 
const MCHAR* TemplateLightClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateLightClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateLight"); 
} 
HINSTANCE TemplateLightClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateLightClassDesc::GetClassDescInstance() 
{
    static TemplateLightClassDesc desc;
    return &desc;
}
...
class TemplateLightParams
    : public ParamBlockDescUtil
{
public:        
    TemplateLightParams()
        : ParamBlockDescUtil(TemplateLightClassDesc::GetClassDescInstance(), _T("TemplateLightParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateLight::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateLight::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateLightParams pblockdesc;
...
{ 
	return "Light template plug-in project."; 
} 
...
{ 
    ...
    static TemplateLightClassDesc classdesc; 
    return &classdesc; 
} 
