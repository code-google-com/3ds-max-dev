...
#include "TemplateHelperObject.h"
...
int TemplateHelperObjectClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateHelperObjectClassDesc::Create(BOOL loading) 
{ 
    return new TemplateHelperObject(); 
} 
const MCHAR* TemplateHelperObjectClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Helper Object"); 
} 
SClass_ID TemplateHelperObjectClassDesc::SuperClassID() 
{ 
    return HELPER_CLASS_ID;
} 
Class_ID TemplateHelperObjectClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x2F8573E4, 0x66F65D3F);
    return cid; 
} 
const MCHAR* TemplateHelperObjectClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateHelperObjectClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateHelperObject"); 
} 
HINSTANCE TemplateHelperObjectClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateHelperObjectClassDesc::GetClassDescInstance() 
{
    static TemplateHelperObjectClassDesc desc;
    return &desc;
}
...
class TemplateHelperObjectParams
    : public ParamBlockDescUtil
{
public:        
    TemplateHelperObjectParams()
        : ParamBlockDescUtil(TemplateHelperObjectClassDesc::GetClassDescInstance(), _T("TemplateHelperObjectParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateHelperObject::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateHelperObject::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateHelperObjectParams pblockdesc;
...
{ 
	return "Helper Object template plug-in project."; 
} 
...
{ 
    ...
    static TemplateHelperObjectClassDesc classdesc; 
    return &classdesc; 
} 
