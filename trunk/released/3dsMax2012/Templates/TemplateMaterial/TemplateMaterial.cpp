...
int TemplateMaterialClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateMaterialClassDesc::Create(BOOL loading) 
{ 
    return new TemplateMaterial(); 
} 
const MCHAR* TemplateMaterialClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Material"); 
} 
SClass_ID TemplateMaterialClassDesc::SuperClassID() 
{ 
    return MATERIAL_CLASS_ID;
} 
Class_ID TemplateMaterialClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x078C8797, 0x19E72EB5);
    return cid; 
} 
const MCHAR* TemplateMaterialClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateMaterialClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateMaterial"); 
} 
HINSTANCE TemplateMaterialClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateMaterialClassDesc::GetClassDescInstance() 
{
    static TemplateMaterialClassDesc desc;
    return &desc;
}
...
class TemplateMaterialParams
    : public ParamBlockDescUtil
{
public:        
    TemplateMaterialParams()
        : ParamBlockDescUtil(TemplateMaterialClassDesc::GetClassDescInstance(), _T("TemplateMaterialParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateMaterial::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateMaterial::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateMaterialParams pblockdesc;
...
{ 
	return "Material template plug-in project."; 
} 
...
{ 
    ...
    static TemplateMaterialClassDesc classdesc; 
    return &classdesc; 
} 
