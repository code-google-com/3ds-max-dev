...
#include "TemplateWorldSpaceObject.h"
...
int TemplateWorldSpaceObjectClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateWorldSpaceObjectClassDesc::Create(BOOL loading) 
{ 
    return new TemplateWorldSpaceObject(); 
} 
const MCHAR* TemplateWorldSpaceObjectClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template World Space Object"); 
} 
SClass_ID TemplateWorldSpaceObjectClassDesc::SuperClassID() 
{ 
    return WSM_OBJECT_CLASS_ID;
} 
Class_ID TemplateWorldSpaceObjectClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x491583C1, 0x4B7C8659);
    return cid; 
} 
const MCHAR* TemplateWorldSpaceObjectClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateWorldSpaceObjectClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateWorldSpaceObject"); 
} 
HINSTANCE TemplateWorldSpaceObjectClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateWorldSpaceObjectClassDesc::GetClassDescInstance() 
{
    static TemplateWorldSpaceObjectClassDesc desc;
    return &desc;
}
...
class TemplateWorldSpaceObjectParams
    : public ParamBlockDescUtil
{
public:        
    TemplateWorldSpaceObjectParams()
        : ParamBlockDescUtil(TemplateWorldSpaceObjectClassDesc::GetClassDescInstance(), _T("TemplateWorldSpaceObjectParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateWorldSpaceObject::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateWorldSpaceObject::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateWorldSpaceObjectParams pblockdesc;
...
{ 
	return "World Space Object template plug-in project."; 
} 
...
{ 
    ...
    static TemplateWorldSpaceObjectClassDesc classdesc; 
    return &classdesc; 
} 
