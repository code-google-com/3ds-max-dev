...
#include "TemplateRenderElement.h"
...
int TemplateRenderElementClassDesc::IsPublic() 
{ 
    return 1; 
} 
void* TemplateRenderElementClassDesc::Create(BOOL loading) 
{ 
    return new TemplateRenderElement(); 
} 
const MCHAR* TemplateRenderElementClassDesc::ClassName() 
{ 
    TODO("Replace with your own localized plug-in name");
    return _M("Template Render Element"); 
} 
SClass_ID TemplateRenderElementClassDesc::SuperClassID() 
{ 
    return RENDER_ELEMENT_CLASS_ID;
} 
Class_ID TemplateRenderElementClassDesc::ClassID() 
{ 
    TODO("Replace the Class_ID with a new unique one")
    static const Class_ID cid = Class_ID(0x55E631C7, 0x4C40A190);
    return cid; 
} 
const MCHAR* TemplateRenderElementClassDesc::Category() 
{ 
    TODO("Replace with an appropriate category")
    return SDK_TUTORIALS_CATEGORY; 
}  
const MCHAR* TemplateRenderElementClassDesc::GetInternalName() 
{ 
    TODO("Replace with your own internal (non-localized) plug-in name");
    return _M("TemplateRenderElement"); 
} 
HINSTANCE TemplateRenderElementClassDesc::HInstance() 
{ 
    return hDllInstance; 
} 
ClassDesc2* TemplateRenderElementClassDesc::GetClassDescInstance() 
{
    static TemplateRenderElementClassDesc desc;
    return &desc;
}
...
class TemplateRenderElementParams
    : public ParamBlockDescUtil
{
public:        
    TemplateRenderElementParams()
        : ParamBlockDescUtil(TemplateRenderElementClassDesc::GetClassDescInstance(), _T("TemplateRenderElementParameters"), 
            IDS_PARAMS, IDD_PANEL, IDS_TITLE)
    {
        TODO("Describe your own parameters here");

        // Adds a float parameter, with a spinner UI
        AddFloatSpinnerParam(TemplateRenderElement::FLOAT_PARAM_ID, _T("Float"), IDS_FLOAT_PARAM, 0.0f, 1000.0f, 0.0f, IDC_PARAM_EDIT, IDC_PARAM_SPIN);

        // Adds a node parameter, with a pick parameter
        AddNodePickParam(TemplateRenderElement::NODE_PARAM_ID, _T("Node"), IDS_NODE_PARAM, IDC_TARGET);
...
static TemplateRenderElementParams pblockdesc;
...
{ 
	return "Render Element template plug-in project."; 
} 
...
{ 
    ...
    static TemplateRenderElementClassDesc classdesc; 
    return &classdesc; 
} 
