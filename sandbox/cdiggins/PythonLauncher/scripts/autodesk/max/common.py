...
def _matrix_to_tuple(mat):
    return (_point3_to_tuple(mat.GetRow(i)) for i in xrange(4))
...
class Mesh(object):                
    def __init__(self, m):
        ...
        self.vertices = tuple(m.Verts[i] for i in xrange(m.NumVerts))
        self.normals = tuple(_point3_to_tuple(m.getFaceNormal(i)) for i in xrange(m.NumFaces))
        uvs = tuple(self._uvcoords())

    def _uvcoords(self):
        for f in xrange(self._mesh.NumFaces):
